...
public class AuthControllerOauthLogin : IAsyncLifetime
{
    private readonly HttpClient _client;
    private readonly Func<Task> _resetDatabase;
    private readonly IdentityServerSettings _identitySettings;

    public AuthControllerOauthLogin(AuthApiFactory apiFactory)
    {
    ...
    private FormUrlEncodedContent CreateLoginRequest(string userName, string password)
    {
        var requestContent = new List<KeyValuePair<string, string>>
        {
            ...
            new KeyValuePair<string, string>("scope", $"{_identitySettings.Scope}")
        };
